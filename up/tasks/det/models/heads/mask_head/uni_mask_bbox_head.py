...
from up.tasks.det.models.postprocess.uni_mask_helper import MaskContourHelper


class MaskPointBboxNet(nn.Module):
    def __init__(self, inplanes, num_classes, cfg):
        super(MaskPointBboxNet, self).__init__()
        self.prefix = 'BboxNet'
    ...
    def get_one_img_gts(self, gt_bboxes, gt_masks):
        new_gt_bboxes = []
        new_gt_masks = []
        for i in range(len(gt_bboxes)):
            bbox_cls = int(gt_bboxes[i, 4])
            bbox = list(map(float, gt_bboxes[i, :4]))
            mask = gt_masks[i]
            valid_contour_masks = self.mask_contour_helper.process(mask, bbox)
            valid_bboxes = self.mask_contour_helper.polygons2bbox(valid_contour_masks, bbox_cls)
            new_gt_bboxes.append(torch.tensor(valid_bboxes).to(gt_bboxes))
            new_gt_masks.extend(valid_contour_masks)

        new_gt_bboxes = torch.cat(new_gt_bboxes, dim=0)

        return new_gt_bboxes, new_gt_masks
    ...
    def get_new_gts(self, input):
        ...
        new_gt_masks = []
        for i in range(len(gt_bboxes)):
            b_new_gt_bbox, b_new_gt_mask = self.get_one_img_gts(gt_bboxes[i], gt_masks[i])
            new_gt_bboxes.append(b_new_gt_bbox)
            new_gt_masks.append(b_new_gt_mask)
    ...
    def forward(self, input):
        output = {}
        if self.training:
            new_gt_bboxes, new_gt_masks = self.get_new_gts(input)
            input['gt_bboxes'] = new_gt_bboxes
            input['gt_masks'] = new_gt_masks
            losses = self.get_loss(input)
            output.update(losses)
            output['gt_bboxes'] = new_gt_bboxes
            output['gt_masks'] = new_gt_masks
        else:
...
@MODULE_ZOO_REGISTRY.register('UniMaskBboxHead')
class MaskPointBboxFC(MaskPointBboxNet):
    def __init__(self, inplanes, feat_planes, num_classes, cfg, num_point=32,
                 ...
                 initializer=None, dropout=0.1, num_mask_point=32):
        super(MaskPointBboxFC, self).__init__(inplanes, num_classes, cfg)

        self.mask_contour_helper = MaskContourHelper(num_contour_points=num_mask_point)
